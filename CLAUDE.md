...
- **Default to no comment**: A comment earns its place only when it records a durable *why* the code itself can't — an invariant, a constraint, a non-obvious trade-off. The comment rules that follow are specific cases of this one. Before committing, reread every comment you added and cut any that restate the code, name a consumer / module / value that could later change, or run longer than the line they annotate. When unsure, cut it — an absent comment never goes stale, conflicts on a rebase, or lies after a refactor.
