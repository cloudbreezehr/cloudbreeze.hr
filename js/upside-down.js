export function initUpsideDown() {
    ...
    (e) => {
      ...
      if (atEdge) {
        ...
        force = Math.min(
          1,
          force +
            (isFlipped ? FORCE_PER_HIT * FORCE_RETURN_MUL : FORCE_PER_HIT),
        );
        updateVisuals();
        ...
        if (
          force >= 1.0 &&
          warningVisible &&
          now - warningShowTime >= WARNING_MIN_MS
        ) {
          triggerFlip();
        }
      }
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      const now = Date.now();
      if (
        touchAccum > TOUCH_DRAG_THRESHOLD &&
        now - lastForceTime >= COOLDOWN
      ) {
        lastForceTime = now;
