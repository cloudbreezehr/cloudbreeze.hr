...
const BLOCK_FRAG_COLORS = [[80, 120, 200], [100, 140, 220], [60, 100, 180]];
...
class Firefly {
  ...
  update() {
    ...
    this.y += this.vy;
    // Wrap
    if (this.x < -FIREFLY_WRAP_MARGIN) this.x += _canvas.width + FIREFLY_WRAP_MARGIN * 2;
    if (this.x > _canvas.width + FIREFLY_WRAP_MARGIN) this.x -= _canvas.width + FIREFLY_WRAP_MARGIN * 2;
    if (this.y < _canvas.height * FIREFLY_CEIL_FRACTION) this.y = _canvas.height * FIREFLY_CEIL_FRACTION + FIREFLY_CEIL_OFFSET;
  ...
  return {
    draw(forces, scrollVelocity, isDarkMode) {
      ...
      for (let i = blockFragments.length - 1; i >= 0; i--) {
        ...
        _ctx.fillRect(-BLOCK_FRAG_SIZE / 2, -BLOCK_FRAG_SIZE / 2, BLOCK_FRAG_SIZE, BLOCK_FRAG_SIZE);
        _ctx.restore();
      }
      ...
      fireflies.forEach(f => {
        ...
        applyAttraction(forces, f, FIREFLY_ATTRACT_RADIUS, FIREFLY_ATTRACT_STRENGTH, FIREFLY_ATTRACT_TANGENT);
    ...
    clickBurst(cx, cy) {
      ...
      for (let i = 0; i < fragCount && blockFragments.length < BLOCK_FRAG_MAX; i++) {
        ...
        blockFragments.push({
          ...
          color: BLOCK_FRAG_COLORS[Math.floor(Math.random() * BLOCK_FRAG_COLORS.length)],
          life: 0, rot: 0,
        });
      }
    },
